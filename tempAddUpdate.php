...
while ($d=mysql_fetch_array($r,MYSQL_ASSOC)){
	...
	if (checkVerify($add) !== true){
		$q1="INSERT into addressVerify (address, user) VALUES ('".addslashes($add)."', '$d[uspsVerify]')";
		@mysql_query($q1);
		echo "<li>$q1</li>";
	}
}
foreach(range('a','e') as $letter){
	...
	while ($d=mysql_fetch_array($r,MYSQL_ASSOC)){
		...
		if (checkVerify($add) !== true){
			$q1="INSERT into addressVerify (address, user) VALUES ('".addslashes($add)."', '".$d["uspsVerify$letter"]."')";
			@mysql_query($q1);
			echo "<li>$q1</li>";
		}
	}
