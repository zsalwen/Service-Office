...
function printSetEV($packet,$def){
	$r=@mysql_query("select otd, name1, name2, name3, name4, name5, name6, address1, city1, state1, zip1, address2, city2, state2, zip2, address3, city3, state3, zip3, address4, city4, state4, zip4, address5, city5, state5, zip5, address6, city6, state6, zip6 from evictionPackets where eviction_id = '$packet'");
	$d=mysql_fetch_array($r, MYSQL_ASSOC);
	$card = $_GET[card];
	$name = "OCCUPANT";
	$line1 = $d["address$def"];
...
if ($_GET[OTD] && $_GET[start] && $_GET[stop]){
	...
	if ($dd > 0){
		...
		$r=@mysql_query($q);
		while($d=mysql_fetch_array($r, MYSQL_ASSOC)){ $i++;
			echo printSetEV($d[eviction_id],1);
		}
	}
}
?>
