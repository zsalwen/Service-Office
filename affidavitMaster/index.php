...
$myFile = "$id.html";
$fh = fopen($myFile, 'w') or die("can't open file");
fwrite($fh, strip($d[LiveAffidavit]));
fclose($fh);
...
fwrite($fh, strip($d[LiveAffidavit]));
fclose($fh);
$cmd1='/usr/local/bin/html2ps '.$id.'.html > '.$id.'.pcl';
$error1=passthru($cmd1,$result1);
$cmd2='/usr/local/bin/html2ps '.$id.'.pcl > '.$id.'.html';
$error2=passthru($cmd2,$result2);
$command = 'python DocumentConverter.py /gitbox/Service-Office/affidavitMaster/'.$id.'.html /gitbox/Service-Office/affidavitMaster/'.$id.'.pdf';
