...
while ($year <= $curYear){
	...
	while ($i < $topMo){$i++;$zi++;
		...
		$r=mysql_query("SELECT eviction_id FROM EVexport WHERE date_received LIKE '%$year-$i2%'");
		...
		//pull BURSON files
		$r=mysql_query("SELECT eviction_id FROM EVexport WHERE date_received LIKE '%$year-$i2%' AND attorneys_id='1'");
		...
		//pull WHITE files
		$r=mysql_query("SELECT eviction_id FROM EVexport WHERE date_received LIKE '%$year-$i2%' AND attorneys_id='3'");
		...
		//pull BGW files
		$r=mysql_query("SELECT eviction_id FROM EVexport WHERE date_received LIKE '%$year-$i2%' AND attorneys_id='70'");
		...
		//pull OTHER files
		$r=mysql_query("SELECT eviction_id FROM EVexport WHERE date_received LIKE '%$year-$i2%' AND attorneys_id <> '1' AND attorneys_id <> '3' AND attorneys_id <> '70'");
...
echo "</tr></table>";*/
$src="http://3.chart.apis.google.com/chart?cht=lc&chs=1000x300&chd=t:".$src."|".$burson."|".$white."|".$bgw."|".$other."&chxl=0:".$src2."|1:|0|$z1|$z2|$z3|$z4|$z&chtt=Exported Evictions Received 2008-$curYear&chdl=All Files|Burson|White|BGW|Others&chco=FF0000,00FF00,0000FF,800080,FF8040";
       ...
       <?=$js?>
        ...
        new google.visualization.LineChart(document.getElementById('visualization')).
            draw(data, {curveType: "function",
                        width: 1000, height: 400, backgroundColor: '#CCBBEE',
