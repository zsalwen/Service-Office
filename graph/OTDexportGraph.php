...
while ($year <= $curYear){
	...
	while ($i < $topMo){$i++;$zi++;
		...
		//pull BURSON files
		$r=mysql_query("SELECT packet_id FROM ps_export WHERE date_received LIKE '%$year-$i2%' AND attorneys_id='1'");
		...
		//pull WHITE files
		$r=mysql_query("SELECT packet_id FROM ps_export WHERE date_received LIKE '%$year-$i2%' AND attorneys_id='3'");
		...
		//pull BGW files
		$r=mysql_query("SELECT packet_id FROM ps_export WHERE date_received LIKE '%$year-$i2%' AND attorneys_id='70'");
		...
		//pull OTHER files
		$r=mysql_query("SELECT packet_id FROM ps_export WHERE date_received LIKE '%$year-$i2%' AND attorneys_id <> '1' AND attorneys_id <> '3' AND attorneys_id <> '70'");
...
echo "</tr></table>";*/
$src="http://3.chart.apis.google.com/chart?cht=lc&chs=1000x300&chd=t:".$src."|".$burson."|".$white."|".$bgw."|".$other."&chxl=0:".$src2."|1:|0|$z1|$z2|$z3|$z4|$z&chtt=Exported Packets Received 2008-$curYear&chdl=All Files|Burson|White|BGW|Others&chco=FF0000,00FF00,0000FF,800080,FF8040";
       ...
       <?=$js?>
        ...
        new google.visualization.LineChart(document.getElementById('visualization')).
            draw(data, {curveType: "function",
                        width: 1200, height: 480, backgroundColor: '#FFFFDD',
                        vAxis: {maxValue: <?=$z?>}, title: 'Exported Packets Received: 2008-<?=$curYear?>',
