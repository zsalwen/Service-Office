...
function id2lender($id){
	$q="SELECT name FROM lenders WHERE lenderID = '$id'";
	$r=@mysql_query($q);
	$d=mysql_fetch_array($r, MYSQL_ASSOC);
	return $d[name];
}
...
function isServer($server,$packet){
	$r=@mysql_query("select server_id from ps_packets where eviction_id='$packet' and (server_id='$server' OR server_ida='$server' OR server_idb='$server' OR server_idc='$server' OR server_idd='$server' OR server_ide='$server')");
	$d=mysql_fetch_array($r, MYSQL_ASSOC);
	if ($d){
		return 1;
	}else{
