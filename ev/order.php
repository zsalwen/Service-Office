...
function colorCode2($hours,$status){
	if ($status != 'SERVICE COMPLETED' && $status != 'CANCELLED'){
...
</td><td align="center"><div style="font-size:30pt" ><?=$hours?> Hours<div><div style="font-size:15pt" ><?=$days?> Days || <span style='background-color:<?=colorCode2($estHours,$d[process_status])?>'>Serve Due: <?=$estFileDate?></span><div></td></tr></table>
</div>
...
<table style="padding:0px; font-weight:bold; border-collapse:collapse; height:150px !important; font-size:small;" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td><a href="../photoDisplay.php?packet=EV<?=$d[eviction_id]?>" target="preview"><?$photoCount=photoCount($d[eviction_id]); echo $photoCount;?> Photo<? if($photoCount != 1){echo "s";}?></a></td>
	</tr>
	<tr>
