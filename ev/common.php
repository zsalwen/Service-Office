...
function talk($to,$message){
 ...
 function timeline($id,$note){
  		@mysql_query("insert into explorer (date,date_time,user,packet,uri) values (NOW(),NOW(),'".$_COOKIE[psdata][name]."','EV$id','$note')") or die(mysql_error());
...
function washURI($uri){
	$return=str_replace('portal//var/www/dataFiles/service/orders/','evictionPackets/',$uri);
	$return=str_replace('data/service/orders/','evictionPackets/',$uri);
