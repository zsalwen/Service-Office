...
function inPackage($id){
	$r=@mysql_query("SELECT name1, name2, name3, name4, name5, name6 from evictionPackets where package_id = '$id' AND process_status <> 'CANCELLED'");
	$files=0;
	$total=0;
...
function packageContents($id){
	...
	$q="select eviction_id, address1, address1a, address1b, address1c, address1d, address1e, client_file, case_no, server_id, server_ida, server_idb, process_status, circuit_court from evictionPackets where package_id = '$id'";
	$r=@mysql_query($q);
	while ($d=mysql_fetch_array($r, MYSQL_ASSOC)){
...
<?
$q="SELECT id, name, set_date from evictionPackages where id != '' order by set_date DESC";
