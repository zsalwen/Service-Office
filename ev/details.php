...
$r=@mysql_query("select *,
						...
						date_format(fileDate, '%W, %M %D %Y') as fileDate
							from evictionPackets where eviction_id = '$_GET[packet]'");
$d=mysql_fetch_array($r,MYSQL_ASSOC);
// some logic questions for building links
//// qc checklist
if ($d[eviction_id] > 3620){
	$checkLink="serviceSheet.php?packet=$d[eviction_id]&autoPrint=1";
}else{
	$checkLink="oldServiceSheet.php?packet=$d[eviction_id]&autoPrint=1";
}
//// otd link
...
?>
<div style="">Service Book for <a href="?packet=<?=$d[eviction_id]?>">Packet EV<?=$d[eviction_id]?></a> <a href="order.php?packet=<?=$d[eviction_id];?>" target="_blank">Edit Order Details</a></div>
<!-- show status -->
<table width="100%" style="border-collapse:collapse;" cellspacing="0" cellpadding="0" border="1">
	...
	<tr>
		...
		<td>
			<table cellspacing="0" cellpadding="2" width="100%">
				<tr>
					<td class="title2">Persons</td>
					<td class="title2">at</td>
					<td class="title2">Addresses</td>
					<td class="title2">by</td>
					<td class="title2">Servers</td>
				</tr>
				<tr>
					<td valign="top">
	...
	<tr>
		...
		<td>
			<li><a href="<?=$checkLink?>" target="_blank">Quality Control checklist</a></li>
			<li><a href="<?=$otdStr?>" target="_blank">Papers to be served</a></li>
		</td>
	</tr>
	<tr>
