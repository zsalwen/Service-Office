...
function serviceSheet($packet){
	...
	$attempt='';
	$attempt=attemptExplode($packet,$i,$address,"FIRST EFFORT");
	if ($attempt != ''){
		$dt=$attempt[0];
		$server=$attempt[1];
	}else{
	...
	$address=strtoupper($d[address1].', '.$d[city1].', '.$d[state1].' '.$d[zip1]);
	$attempt='';
	$attempt=attemptExplode($packet,$i,$address,"SECOND EFFORT");
	if ($attempt != ''){
		$dt=$attempt[0];
		$server=$attempt[1];
	}else{
	...
	$address=strtoupper($d[address1].', '.$d[city1].', '.$d[state1].' '.$d[zip1]);
	$attempt='';
	$attempt=attemptExplode($packet,$i,$address,"POSTING DETAILS");
	if ($attempt != ''){
		$dt=$attempt[0];
		$server=$attempt[1];
	}else{
	...
	?>
		...
		<tr>
			<td colspan="4" align="center"><?=historyList($_GET[packet],$i,$d[attorneys_id])?></td>
		</tr>
	</table>
	</fieldset>
	<? }
	...
	if (trim($d[accountingNotes]) != ''){
		echo "<table width='80%' align='center' border='1' style='border-collapse:collapse;'><tr><td align='center'><b>ACCOUNTING NOTES</b><br>".strtoupper(stripslashes($d[accountingNotes]))."</td></tr></table>";
	}
	?>
	</td></tr></table>
