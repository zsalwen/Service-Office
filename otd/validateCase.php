...
@mysql_query("update ps_packets set caseVerify='".$_COOKIE[psdata][name]."' where packet_id='$_GET[validate]'") or die(mysql_error());
...
if ($court == "MONTGOMERY"){
	...
	if ($len == 7){ 
		...
		if (!preg_match('/V/',$char[6]) && !preg_match('/D/',$char[6])){ alert("Invalid Character: $char[6], expecting D or V. ($case)"); }
