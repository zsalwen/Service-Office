...
function oosList($packet){
	...
	if ($d[state1] != '' && strtoupper($d[state1]) != 'MD' && $d[server_id] != '' && id2state($d[server_id]) != 'MD' && $d[server_id] != 218){
		$list .= "<li>".id2name($d[server_id])." - <div style='font-size:10px; text-align:right; line-height:0px; border:solid 1px black; display:inline;'>".strtoupper($d[address1])."<br>".strtoupper($d[city1]).", ".strtoupper($d[state1])." ".$d[zip1]."</div></li>";
	}
	foreach(range('a','e') as $letter){
		if ($d["state1$letter"] != '' && strtoupper($d["state1$letter"]) != 'MD' && $d["server_id$letter"] != '' && id2state($d["server_id$letter"]) != 'MD' && $d["server_id$letter"] != 218){
			$list .= "<li>".id2name($d["server_id$letter"])." - <div style='font-size:10px; text-align:right; line-height:0px; border:solid 1px black; display:inline;'>".strtoupper($d["address1$letter"])."<br>".strtoupper($d["city1$letter"]).", ".strtoupper($d["state1$letter"])." ".$d["zip1$letter"]."</div></li>";
		}
	}
	if ($list != ''){
		$list = "<ol>$list</ol>";
