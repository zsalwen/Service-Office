...
function EVpackageFile($package_id, $file_id, $contractor_rate){
	timeline($file_id,$_COOKIE[psdata][name]." Packaged Order");
...
function EVmakePackage($array1,$array2,$package_id){
//	echo "Package ID :: $package_id";
//	echo "Client Rate :: $array2[0]<br>";
//	echo "Contractor Rate :: $array3[0]<br>";
//	echo "for file id's (the foreach loop went here) :: ";
	foreach ($array1 as $id) {
		checkPay($id,'EV');
		EVpackageFile($package_id,$id,$array2[0]);
		//echo "$id ";
	}
}
...
if ($_POST[submit]){
	if (!$_POST[package][contractor] || !$_POST[server_id]){
		...
		makePackage($_POST[package]['otd'],$_POST[package]['contractor'],$_POST[package]['contractora'],$_POST[package]['contractorb'],$_POST[package]['contractorc'],$_POST[package]['contractord'],$_POST[package]['contractore'],$packageID);
		hardLog('Created OTD package '.$packageID,'user');

		//monitor('Your package "' .$_POST[name]. '" has been created.');
		foreach ($_POST[package]['otd'] as $value){
			...
			hardLog('Dispatched file OTD'.$value,'user');
		...
		$r1=@mysql_query($q1) or die("Query: $q1<br>".mysql_error());
		$packageID = mysql_insert_id();	
		EVmakePackage($_POST[package]['ev'],$_POST[package]['contractor'],$_POST[package]['contractora'],$packageID);
		hardLog('Created EV package '.$packageID,'user');

		//monitor('Your package "' .$_POST[name]. '" has been created.');
		foreach ($_POST[package]['ev'] as $value){
			...
			hardLog('Dispatched file EV'.$value,'user');
			ev_timeline($value,$_COOKIE[psdata][name]." Dispatched Order, Deadline: $_SESSION[estFileDate]");
		}
		$packageName=$fileCount.initals(id2name($_POST[server_id]));
		$packageName = $packageName.date('mdY-H:i:s');
