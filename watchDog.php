...
$rOut = @mysql_query("select * from watchDog where status <> 'Search Complete'");
while ($dOut=mysql_fetch_array($rOut,MYSQL_ASSOC)){
	...
	if (!$dOut['response'] && !$dOut['status']){
		@mysql_query("UPDATE watchDog set status = 'Case Watch Started', response = '$response', watchStart = NOW(), lastChecked = NOW() where watchID = '$id'");
		...
		@mysql_query("UPDATE watchDog set status = 'New Case Found', lastResult = '$response', lastChecked = NOW() where watchID = '$id'");
		...
		@mysql_query("UPDATE watchDog set status='Searching...', lastChecked = NOW(), lastResult = '$response' where watchID = '$id'");
	}
}
