...
function colorCode3($total,$est){
	//$total is total datediff from date_received to estFileDate, $est is datediff from estFileDate to current date
	$current=$total-$est;
	$ratio=255/$total;
	if ($est <= -24){
		//black (0,0,0)
		return "0,0,0; color:FFFFFF !important;";
	}
	if ($est > -24 && $est <= 0){
		//red (255,0,0) to black (0,0,0)
		$color=",0,0";
		$rem=round($current*$ratio);
		return $rem.$color."; color:000000 !important;";
	}
	if ($est > 0 && $est <= 48){
		//yellow(255,255,0) to red (255,0,0)
		$color="";
		$rem=round($est*$ratio);
		return "255,".$rem.",0; color:000000 !important;";
	}
	if ($est > 48){
		//green (0,255,0) to yellow (255,255,0)
		$color="0,255,0"; 
		$rem=round($current*$ratio);
		return $rem.",255,0; color:000000 !important;";
	}
}
...
function presaleActiveList($id,$letter){ $_SESSION[active]++;
	...
	while ($d=mysql_fetch_array($r,MYSQL_ASSOC)){ $_SESSION[active2]++;
		...
		//get background-color with gradientCode, using $d[totalHours]
...
$data='<ol>';
$r=@mysql_query("select eviction_id, date_received, request_close, affidavit_status, service_status, circuit_court, dispatchDate, attorneys_id, estFileDate, TIMEDIFF( NOW(), date_received) as hours, DATEDIFF(estFileDate, CURDATE()) as estHours, DATEDIFF(estFileDate,SUBSTR(date_received,1,10)) as totalHours from evictionPackets where server_id='$id' and (process_status = 'Assigned' OR process_status = 'ASSIGNED') order by  eviction_id");
while ($d=mysql_fetch_array($r,MYSQL_ASSOC)){ $_SESSION[active2]++;
	$estHours=($d[estHours]*24)-date('G');
	$totalHours=($d[totalHours]*24);
	$_SESSION[active3]++;
