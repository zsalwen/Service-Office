...
/*
$Id: nusoap.php,v 1.1 2009/10/30 14:33:49 patrick Exp $
...
* @version  $Id: nusoap.php,v 1.1 2009/10/30 14:33:49 patrick Exp $
* @access   public
*/
class nusoap_base {
	/**
...
class soap_transport_http extends nusoap_base {
	...
	var $certRequest = array();	// keys must be cainfofile (optional), sslcertfile, sslkeyfile, passphrase, certpassword (optional), verifypeer (optional), verifyhost (optional)
								// cainfofile: certificate authority file, e.g. '$pathToPemFiles/ca.pem'
	...
	*/
    function parseRequest($headers, $data) {
		...
		// else successfully parsed request into soapval object
			...
			// get/set methodname
			$this->methodURI = $parser->_struct_namespace;
			$this->methodname = $parser->_struct_name;
...
class wsdl extends nusoap_base {
	// URL or filename of the  of this WSDL
    var $wsdl; 
...
class nusoap_parser extends nusoap_base {
	...
	function nusoap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			if(!xml_parse($this->parser,$xml,true)){
				...
				$this->debug('parsed successfully, found  struct: '.$this->_struct.' of name '.$this->_struct_name);
				// get final value
				$this->soapresponse = $this->message[$this->_struct]['result'];
				// get header value
				if($this->_header != '' && isset($this->message[$this->_header]['result'])){
					$this->soapheader = $this->message[$this->_header]['result'];
				}
				// resolve hrefs/ids
	...
	*/
	function start_element($parser, $name, $attrs) {
		...
		// depth = how many levels removed from ?
		...
		// set method
			...
			$this->status = 'method';
			$this->_struct_name = $name;
			$this->_struct = $pos;
			$this->message[$pos]['type'] = 'struct';
			$this->debug("found  struct $this->_struct_name, pos $this->_struct");
		}
		// set my status
		...
		foreach($attrs as $key => $value){
			...
			} elseif ($key != 'href' && $key != 'xmlns' && $key_localpart != 'encodingStyle' && $key_localpart != '') {
			...
			// 
			if($key_localpart == '' && $value == 1){
				$this->status = 'method';
				$this->_struct_name = $name;
				$this->_struct = $pos;
				$this->debug("found  struct $this->_struct_name, pos $pos");
			}
            // for doclit
            $attstr .= " $key=\"$value\"";
		}
	...
	*/
	function end_element($parser, $name) {
        ...
        } elseif($pos >= $this->_struct){
        	$this->document .= "</" . (isset($prefix) ? $prefix . ':' : '') . "$name>";
        }
		// switch status
		if($pos == $this->_struct){
			$this->status = 'body';
			$this->_struct_namespace = $this->message[$pos]['namespace'];
		} elseif($name == 'Body'){
			$this->status = 'envelope';
		 } elseif($name == 'Header'){
			$this->status = 'envelope';
...
class nusoap_client extends nusoap_base  {
	...
	*			              in which case there
	*                         is really one parameter, the  of the fragment
