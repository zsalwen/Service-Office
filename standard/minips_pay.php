...
$r=@mysql_query("select * from standard_packets where $field = '$string'");
$c=mysql_num_rows($r);
if ($c == 1){
...
function getPayInstructions($attID,$prefix){
	$q = "SELECT payInstructions FROM attorneys WHERE attorneys_id = '$att_id'";
...
hardLog('updated payment information for '.$_GET[id],'user');
	...
	$q1 = "UPDATE standard_packets, ps_pay SET 
									...
									standard_packets.accountingNotes='".addslashes($_POST[accountingNotes])."'
										WHERE packet_id='$_POST[id]' AND standard_packets.packet_id=ps_pay.packetID AND ps_pay.product='S'";		
...
$q1 = "SELECT * FROM standard_packets, ps_pay WHERE standard_packets.packet_id = $_GET[id] AND standard_packets.packet_id=ps_pay.packetID AND ps_pay.product='S'";
$r1 = @mysql_query ($q1) or die(mysql_error());
...
<? } ?>
<a href="http://staff.mdwestserve.com/standard/ps_write_invoice.php?id=<?=$data[packet_id];?>" target="_Blank">PS Write Invoice</a>
<? if ($_GET[qc]){ ?>
...
<iframe height="200px" width="750px"  frameborder="0" src="http://staff.mdwestserve.com/acctNotes.php?standard=<?=$data[packet_id]?>"></iframe>
</fieldset>
</form>
</td></tr>
</table>
