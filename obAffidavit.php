...
error_log("[".date('h:iA n/j/y')."] [".$_COOKIE[psdata][name]."] [".trim($_GET[packet])."] [makeAffidavit($p,$defendant,$level,$user_id]  \n", 3, '/logs/fail.log');
	...
	if ($iIDd){
		...
		$historyd = "<div style='font-weight:300'><u>Describe with particularity the good faith efforts to serve the mortgagor or grantor, ".$d1["name$def"].",  by personal delivery:</u></div>
				".$attemptsd;
	 ?>
		<table style='page-break-after:always; padding:0px;' cellpadding="0" cellspacing="0" width="100%" height="600px" cellpadding="0" cellspacing="0" align="center" bgcolor="#FFFFFF" <? if (strtoupper($d1[affidavit_status]) != "SERVICE CONFIRMED"){echo $dim;}?>>
	<?
	...
	<? if ($residentDesc){
		...
		<tr cellpadding='0' cellspacing='0'>
			<td cellpadding='0' cellspacing='0' colspan="2" style="padding-left:20px; padding-top:0px; padding-bottom:0px; line-height:2;"><font size='4'><b><?=stripslashes($delivery)?></b></font></td>
		</tr>       
		<tr cellpadding='0' cellspacing='0'>
