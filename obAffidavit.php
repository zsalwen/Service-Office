...
error_log("[".date('h:iA n/j/y')."] [".$_COOKIE[psdata][name]."] [".trim($_GET[packet])."] [makeAffidavit($p,$defendant,$level,$user_id]  \n", 3, '/logs/fail.log');
	...
	<font size='5'>Circuit Court for ".$court."</font></td></tr>
		<tr cellpadding='0' cellspacing='0'><td cellpadding='0' cellspacing='0' class='a' width='75%' style='padding-top:0px; padding-bottom:0px;'><font size='2'>".$plaintiff."<br /><small>_____________________<br /><em>Plaintiff</em></small><br /><br />v.<br /><br />";
			...
			$header .= "<small>_____________________<br /><em>Defendant</em></small></font></td>
				<td cellpadding='0' cellspacing='0' align='right' width='25%' valign='top' style='padding-left:200px; padding-top:0px; padding-bottom:0px;' nowrap='nowrap'><div style='border:solid 1px #666666;'><center><font size='5'>Case Number<br />&nbsp;".str_replace(0,'&Oslash;',$d1[case_no])."</font></center></div>";
	...
	if ($type == "non"){
		...
		if ($attempts != ''){
			...
			$history2 = "<u><font-size='2'>Include the date of the posting and a description of the location of the posting on the property:</font></u><b>".$posting."</b>";
		if ($mailing == ''){
	...
	?>
		<table style='border-collapse:collapse; page-break-after:always; padding:0px;' cellpadding="0" cellspacing="0" width="100%" align="center" bgcolor="#FFFFFF" <? if (strtoupper($d1[affidavit_status]) != "SERVICE CONFIRMED"){ echo $dim;}?>>
	...
	if ($iIDd){
		...
		$historyd = "<u>Describe with particularity the good faith efforts to serve the mortgagor or grantor, ".$d1["name$def"].",  by personal delivery:</u>
				<b>".$attemptsd."</b>";
	 ?>
		<table style='border-collapse:collapse; page-break-after:always; padding:0px;' cellpadding="0" cellspacing="0" width="100%" cellpadding="0" cellspacing="0" align="center" bgcolor="#FFFFFF" <? if (strtoupper($d1[affidavit_status]) != "SERVICE CONFIRMED"){echo $dim;}?>>
	<?
	...
	$cord=$d1[packet_id]."-".$def."-".$serverID."%";
	 echo "<tr cellpadding='0' cellspacing='0'>".$header."<IMG SRC='http://staff.mdwestserve.com/barcode.php?barcode=".$cord."&width=300&height=40'><center>File Number: ".$d1[client_file]."<br />[PAGE]</center></td></tr>"; ?>
		<tr cellpadding='0' cellspacing='0'>
			...
			<center><font size='4'><b><?=$result?></b></font></center>
			<font size='2'>Pursuant to Maryland Real Property Article 7-105.1 and Maryland Rules of Procedure <?=$article?> <?=$result?> a copy of the <?=$addlDocs?> and all other papers filed with it (the "Papers") in the above-captioned case by:</font><br>
			<div style='padding-left:20px;'><?=stripslashes($history)?></div>
	<?
	if ($iID == $iiID){
	}else{
	?>        
	...
	?> 
		...
		<tr cellpadding='0' cellspacing='0'>
			...
			<font size='2'>Pursuant to Maryland Real Property Article 7-105.1 and Maryland Rules of Procedure <?=$article?> <?=$result?> a copy of the <?=$addlDocs?> and all other papers filed with it (the "Papers") in the above-captioned case by:</font><br>
		<? } ?>
		<div style='padding-left:20px;'><?=stripslashes($history2)?></div>
	...
	ob_start();
	?>
	<table style='border-collapse:collapse; border-collapse:collapse; page-break-after:always; padding:0px;' cellpadding="0" cellspacing="0" width="100%" align="center" bgcolor="#FFFFFF" <? if (strtoupper($d1[affidavit_status]) != "SERVICE CONFIRMED"){echo $dim;}?>>
	<?
